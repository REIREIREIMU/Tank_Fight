...
{
    image_ = LoadGraph("Assets/Reticle.png");

    bgmHandle = LoadSoundMem("sound/BGM.mp3");
    bgm_volume = 140;

    // BGM再生
    ChangeVolumeSoundMem(bgm_volume, bgmHandle);
...
void PlayScene::PlayerDeath()
{
    ...
    if (!player->IsAlive()) {
        ...
        DeleteSoundMem(bgmHandle); // BGM停止
...
void PlayScene::DrawUI()
{
    ...
    // UI表示
    {
        ...
        DrawExtendGraph(mouseX - ImgSize, mouseY - ImgSize,
            mouseX + ImgSize, mouseY + ImgSize, image_, TRUE);

        //DrawCircle(mouseX, mouseY, 10, GetColor(255, 0, 0), FALSE);
    }
}